...
The paper starts from a practical observation: modern CNIs now implement overlay tunneling, transport, security, and application-layer policy, so container networking has become a substantial infrastructure tax. The authors measure Cilium and show that tunneled 100GbE transport can consume `6.6` host cores, Envoy-style HTTP processing reaches `6.4` cores at `10K` requests per second, and software security paths still fail to approach line rate.

SmartNIC offload seems like the obvious fix, but the existing interface is wrong for containers. SR-IOV and NVIDIA Sub-Functions expose packet-oriented L2 devices, scale only to hundreds of virtual devices because queues and interrupts are provisioned per device, and virtualize PCIe-facing resources too coarsely to share DMA and crypto accelerators at per-message granularity.

## Key Insight

The key claim is that socket granularity is the right abstraction boundary. If the virtual device is exposed as a stream-oriented socket endpoint, the SmartNIC can offload both below-socket work such as tunneling and transport and above-socket work such as encryption or HTTP processing without repeatedly translating messages into packets and back again. SG-IOV makes that practical by separating control from payload handling: SmartNIC-side software cores track ring-buffer state and generate jobs, while DMA, RDMA, inline engines, and look-aside accelerators touch the data.

## Design

SG-IOV centers on a `warp pipe`: source and sink ring buffers plus a transformation between them. A warp pipe can connect host memory to SmartNIC memory, chain stages together, or span machines, so an I/O virtual device becomes a composable stream-processing path rather than a packet queue pair.

The signal plane keeps buffer state synchronized. SG-IOV introduces `Cross-FIFO`, a lightweight full-duplex signaling structure inspired by UART FIFOs, and multiplexes many virtual devices over one signaling channel instead of assigning each device its own queue pair. Each `64-bit` signal carries a type, ring-buffer id, and head/tail update, letting the backend demultiplex state changes and generate work for the right socket. Job generation must also handle both size-preserving DMA-style operations and size-varying ones such as encryption over bounded ring buffers, so the signal plane recursively splits wrap-around cases into contiguous chunks and uses in-place chunking for one tricky size-decreasing case.

The data plane enforces fine-grained virtualization. Each warp pipe gets a FIFO job queue; the scheduler supports round-robin, strict priority, and a Dominant Resource Fairness variant specialized to the paper's equal-bandwidth pipeline setting. Executors remain stateless and include full-copy DMA, delegator-initialized zero-copy RDMA, inline engines for VxLAN/IPsec-style work, and look-aside or in-motion crypto. The prototype uses BlueField-3 PCIe device emulation plus mediated pass-through devices and exposes the result as `SGIOV-CNI`.

## Evaluation

The evaluation connects microbenchmarks to end-to-end CNI behavior on two `100GbE` servers with BlueField-3 cards. The baselines are Cilium `v1.16` and an SR-IOV/Sub-Function setup extended with NVIDIA's DOCA VNF stack.
